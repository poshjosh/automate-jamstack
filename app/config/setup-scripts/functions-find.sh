...
find_and_replace_env_variables_in_text() {
    ...
    trace "    ENTERED find_and_replace_env_variables_in_text   text=$1"
    ...
    remove_file $temp_env_file

    return_value_find_and_replace_env_variables_in_text=$holder

    debug "    EXITING find_and_replace_env_variables_in_text   input=$1   output=$return_value_find_and_replace_env_variables_in_text"
}
...
find_and_replace_text_in_dir() {
    ...
    if [ -d "./$tgt" ]; then
        ...
        warning "Directory not found. Find and replace will not be carried out for $tgt"
    fi
}
