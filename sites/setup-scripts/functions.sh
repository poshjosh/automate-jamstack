...
ensure_node_modules() {
    ...
    local global_node_modules_dir="./node_modules"
    local site_node_modules_dir="${SITE_DIR}/node_modules"
    if [ ! -d "$site_node_modules_dir" ]; then

        if [ -d "$global_node_modules_dir" ]; then
            cp ${global_node_modules_dir} ${SITE_DIR}
        fi

        if [ -f "./package.json" ]; then
            cp "./package.json" ${SITE_DIR}
        fi

        if [ -f "./yarn.lock" ]; then
            cp "./yarn.lock" ${SITE_DIR}
        fi

        if [ -f "./package-lock.json" ]; then
            cp "./package-lock.json" ${SITE_DIR}
        fi

        # Whether the above succeed or not we just go ahead
        local msg="Installing node modules"
        echo $msg
        # yarn global triggers yarn install
        (cd ${SITE_DIR} && (yarn global add gatsby-cli && echo "  SUCCESS: $msg" || echo "  ERROR: $msg"))
...
delete_node_modules() {
    if [ -d "${SITE_DIR}" ]; then
        ...
        echo "Cleaning site directory/folder: ${SITE_DIR}"

        if [ -f "${SITE_DIR}/package.json" ]; then
            rm -Rf "${SITE_DIR}/package.json";
        fi

        if [ -f "${SITE_DIR}/package-lock.json" ]; then
            rm -Rf "${SITE_DIR}/package-lock.json";
        fi

        if [ -f "${SITE_DIR}/yarn.lock" ]; then
            rm -Rf "${SITE_DIR}/yarn.lock";
        fi
...
#    nohup gatsby develop -H 0.0.0.0 -p ${APP_PORT}  WORKED .. but blocks.. nothing after executes
    (cd ${SITE_DIR} && gatsby develop -H 0.0.0.0 -p ${APP_PORT}) # this blocks.. nothing after executes
}

gatsby_build() {
    local msg='Building site'
    echo $msg
    (cd $SITE_DIR && yarn run build && echo "    SUCCESS: $msg" || echo "    ERROR: $msg")
}
...
#    aws s3api put-bucket-policy --bucket <AWS_S3_BUCKET_NAME> --policy file://policy.json
    ...
    # files
    aws s3 cp ${SITE_DIR}/public/ s3://${AWS_S3_BUCKET_NAME}/ --recursive
}
...
gatsby_plugin_s3_deploy() {
    ...
    if [ "$VERBOSE" = true ]; then
        (cd ${SITE_DIR} && yarn run deploy -y)
    else
        (cd ${SITE_DIR} && yarn run deploy --silent -y)
    fi
...
gatsby_setup() {
    ...
    local gatsby_node_module_dir="${SITE_DIR}/node_modules/gatsby-cli"
